...
docker build -t multi-layer-scc-petclinic -f Dockerfile_petclinic_openj9_scc_app_cache .
echo ""

echo -n "Shared Class Cache Size : "
SCC_SIZE=$(docker run --rm -it multi-layer-scc-petclinic /bin/du -sh /opt/java/.scc/ | cut -d ' ' -f 1 | tr -d '\040\011\012\015')
echo "${SCC_SIZE}"
echo ""

echo -n "Starting petclinic with scc ... "
container_one=$(docker run --rm --cpus 1 -m256m -p 10080:8080 -d multi-layer-scc-petclinic)
echo "Done."
echo ""

echo -n "Starting petclinic without scc ... "
container_two=$(docker run --rm -e OPENJ9_JAVA_OPTIONS="-Xshareclasses:none" --cpus 1 -m256m -p 10081:8080 -d multi-layer-scc-petclinic)
echo "Done."
echo ""
...
echo -n "Petclinic Startup with App SCC : "
